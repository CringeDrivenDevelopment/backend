...
func (s *Service) Create(ctx context.Context, title string, playlistType queries.PlaylistType) (DtoPlaylist, error) {
	tx, txErr := s.pool.BeginTx(ctx, pgx.TxOptions{})
	if txErr != nil {
		return DtoPlaylist{}, txErr
	}
...
func (s *Service) GetById(ctx context.Context, playlistId string, userId int64) (DtoPlaylist, error) {
	...
	tracks := make([]track.DtoTrack, len(playlist.Tracks))
...
func (s *Service) GetAll(ctx context.Context, userId int64) ([]DtoPlaylist, error) {
	q := queries.New(s.pool)
	...
	result := make([]DtoPlaylist, len(playlists))
	for i, playlist := range playlists {
		count := playlist.Count.Int32
		allowedCount := playlist.AllowedCount.Int32
		time := playlist.Time

		result[i] = DtoPlaylist{
			Id:           playlist.ID,
			Title:        playlist.Title,
			Thumbnail:    playlist.Thumbnail,
			Count:        int(count),
...
func (s *Service) Rename(ctx context.Context, playlistId, title string, userId int64) error {
	tx, txErr := s.pool.BeginTx(ctx, pgx.TxOptions{})
	if txErr != nil {
		return txErr
	}
...
func (s *Service) UpdatePhoto(ctx context.Context, playlistId, thumbnail string, userId int64) error {
	tx, txErr := s.pool.BeginTx(ctx, pgx.TxOptions{})
	if txErr != nil {
		return txErr
	}
...
func (s *Service) Delete(ctx context.Context, playlistId string) error {
	tx, txErr := s.pool.BeginTx(ctx, pgx.TxOptions{})
	if txErr != nil {
		return txErr
	}
