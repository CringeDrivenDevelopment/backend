...
func (s *Auth) VerifyToken(authHeader string) (int64, error) {
	...
	if tokenStr == "" {
		return 0, huma.Error401Unauthorized("empty Authorization header")
	}
	...
	if !token.Valid || token.Method != jwt.SigningMethodHS256 {
		return 0, huma.Error401Unauthorized("invalid token")
	}
...
func (s *Auth) ParseInitData(initDataRaw string) (int64, error) {
	if initdata.Validate(initDataRaw, s.botToken, s.expires) != nil {
		return 0, huma.Error401Unauthorized("invalid init data")
	}

	initDataValues, err := url.ParseQuery(initDataRaw)
	if err != nil {
		return 0, huma.Error400BadRequest("failed to parse url query")
	}

	initDataUser := initDataValues.Get("user")

	if initDataUser == "" {
		return 0, huma.Error400BadRequest("telegram user empty")
	}

	user := models.TelegramData{}
	err = sonic.Unmarshal([]byte(initDataUser), &user)
	if err != nil {
		return 0, huma.Error400BadRequest("failed to parse user data")
	}
